import { sendTextMessage, sendTemplateMessage, sendInteractiveList } from '@/lib/whatsapp/meta-api'
...
// ------------------------------------------------------------
// Interactive list (send_interactive_menu automation step)
// ------------------------------------------------------------

export interface SendInteractiveListEngineArgs {
  accountId: string
  userId: string
  conversationId: string
  contactId: string
...
export async function engineSendInteractiveList(args: SendInteractiveListEngineArgs): Promise<void> {
  const db = supabaseAdmin()
  ...
  const attempt = async (phone: string): Promise<string> => {
    const r = await sendInteractiveList({
      ...
      sections: [{
        title: args.sectionTitle ?? 'Options',
        rows: args.rows,
      }],
    })
    return r.messageId
